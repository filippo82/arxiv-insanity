...
def copy_processed_file_to_bucket():
    gcp_cloud_storage_bucket_block = GcsBucket.load("block-bucket-arxiv-data")
...
def log_flow(name: str = "Jake"):
    logger = get_run_logger()
    log_task(name)
    download_dataset()
    prepare_json_for_bigquery()
    path = copy_processed_file_to_bucket()
    logger.info(f"The procesed file has been stored at {path}")
